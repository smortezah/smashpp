...
static i32 GetFirstNonActiveRM (RCLASS* C) {
  for (i32 k=0 ; k < C->mRM ; ++k)
    if (C->active[k] == 0)
      return k;
...
void StartRMs (RCLASS* C, HASH* H, u64 relative, u64 absolute, u64 idx, u8 ir) {
  ENTRY *E;

  if((E = GetHEnt(H, idx)) == NULL)
    return; // NEVER SEEN IN THE HASH TABLE, SO LETS QUIT

  for (u32 n=0; C->nRM < C->mRM && n < E->nPos;) {
    u32 k = GetFirstNonActiveRM(C);
...
// PRINT BLOCK
void PrintBlock (RCLASS* C, HEADERS* Head, u64 relative, u64 absolute, u32 n,
                 u8* cName, FILE* W) {
  if (C->RM[n].rev == 0) { // REGULAR REPEAT
    u64 idxPos = GetIPoint(Head, C->RM[n].init-C->kmer);
...
// STOP USELESS REPEAT MODELS
void StopRMs
(RCLASS* C, HEADERS* Head, u64 position, u64 absolute, u8* buf, FILE* Writter) {
  i32 largerRM = -1, largerRMIR = -1;
  u64 size = 0, sizeIR = 0;

  if (C->nRM > 0) {
    for (i32 id=0 ; id < C->mRM; ++id) {
...
void ResetAllRMs (RCLASS* C, HEADERS* Head, u64 relative, u64 absolute,
                  u8* cName, FILE* Writter) {
  i32 largerRM=-1, largerRMIR=-1;
  u64 size=0, sizeIR=0;
