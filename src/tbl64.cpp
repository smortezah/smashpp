...
auto Table64::query_counters(Table64::ctx_t l) const -> std::array<Table64::val_t, CARDIN> {
  auto row_address = &tbl[l];
  return {*row_address, *(row_address + 1), *(row_address + 2), *(row_address + 3)};
}

#ifdef DEBUG
void Table64::dump(std::ofstream& ofs) const { ofs.write((const char*)&tbl[0], tbl.size()); }

void Table64::load(std::ifstream& ifs) const { ifs.read((char*)&tbl[0], tbl.size()); }

uint64_t Table64::count_empty() const {
  return static_cast<uint64_t>(std::count(std::begin(tbl), std::end(tbl), 0));
}

uint64_t Table64::max_tbl_val() const { return *std::max_element(std::begin(tbl), std::end(tbl)); }
