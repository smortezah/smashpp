...
std::string SVG::begin_elem (std::string name) const {
  return "<" + name + " ";
}

std::string SVG::mid_elem () const {
  return ">\n";
}
...
std::string SVG::end_empty_elem () const {
  return "/>\n";
}
...
void SVG::print_header (std::ofstream& f) const {
  ...
  if (width!=0.0f && height!=0.0f)
    f << 
      attr("viewBox", "0 0 "+std::to_string(width)+" "+std::to_string(height));
  else
...
void Text::plot (std::ofstream& f) const {
  f << begin_elem("text")
    << attr("id", std::to_string(x)+std::to_string(y))
    << attr("x", x, true)
    << attr("y", y, true);
  if (dx != 0.0f)  
    f << attr("dx", dx, true);
...
std::string Path::C (float x1, float y1, float x2, float y2, float x, float y)
const {
  std::stringstream ss;
...
void Pattern::set_tail (std::ofstream& f) const {
  f << end_elem("pattern");
}
...
void Defs::set_tail (std::ofstream& f) const {
  f << end_elem("defs");
}
...
void FilterSVG::set_tail (std::ofstream& f) const {
  f << end_elem("filter");
}
...
void FeMerge::set_tail (std::ofstream& f) const {
  f << end_elem("feMerge");
}
