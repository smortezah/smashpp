...
inline void Filter::set_wsize (const Param& p) {
  if (p.manFilterScale) {
    ...
    case FilterScale::S:  wsize=std::pow(2, 2*lg)   + 1;  break;
    case FilterScale::M:  wsize=std::pow(2, 2*lg+1) + 1;  break;
    case FilterScale::L:  wsize=std::pow(2, 2*lg+2) + 1;  break;
    }
  } else {
...
inline void Filter::show_info (const Param& p) const {
  ...
  constexpr uint8_t tblWidth=60;//static_cast<u8>(lblWidth+Ms.size()*colWidth);
  ...
  toprule();
  label("Files");       header("Size (B)");   header("Name");   std::cerr<<'\n';
  midrule();
  label("Reference");   file_vals('1');       file_vals('r');   std::cerr<<'\n';
  label("Target");      file_vals('2');       file_vals('t');   std::cerr<<'\n';
  botrule();
}
...
void Filter::smooth_seg (const Param& p) {
  ...
  std::cerr << message << "finished. " << "Detected " << nSegs 
            << " segment" << (nSegs==1 ? "" : "s") << ".\n";
}
...
inline void Filter::blackman () {
  ...
  for (auto n=(wsize+1)>>1u, last=wsize-1; n--;)
    window[n] = window[last-n] = static_cast<float>(
      0.42 - 0.5*std::cos(n*num1/den) + 0.08*std::cos(n*num2/den));
  if (window.front() < 0)
    window.front() = 0.0; // Because of low precision
  if (window.back() < 0)
    window.back() = 0.0;  // Because of low precision
}
...
inline void Filter::smooth_seg_rect (const Param& p) {
  ...
  const auto jump_lines = [&]() {
    for (uint64_t i=p.sampleStep-1; i--;)  
  ...
  posF.close();
  if (file_is_empty(positionName))  
    remove(positionName.c_str());
  filF.close();
  if (!SaveFilter)
...
inline void Filter::smooth_seg_non_rect (const Param& p) {
  ...
  // First value
  for (auto i=(wsize>>1u)+1; i-- && std::getline(prfF,num); jump_lines()) {
  ...
  for (auto i=1u; i!=(wsize>>1u)+1; ++i) {
    ...
    if (++idx < wsize+1)
      ...
      sum = std::inner_product(seqBeg+(idx%wsize), seqBeg+offset, winBeg, 0.f);
    ++seg->pos;
...
void Filter::merge_extract_seg (uint32_t ID, std::string ref, std::string tar) 
const {
  ...
  for (std::string beg,end,ent; posF>>beg>>end>>ent; ++i) {
    ...
    if (std::stoull(end) > maxTarPos)
      subseq->size = static_cast<std::streamsize>(maxTarPos-subseq->begPos+1);
    else
      subseq->size = static_cast<std::streamsize>(
        std::stoull(end)-subseq->begPos+1);
...
std::string dest) const {
  ...
  std::ofstream fmid(LBL_MID+"-"+gen_name(ID, origin, dest, Format::POSITION));
  int i {0};

  for (std::string begDir, endDir, entDir, selfEntDir; 
       fDirect>>begDir>>endDir>>entDir>>selfEntDir; ++i) {
    const std::string refRev {
      gen_name(ID, origin, dest, Format::SEGMENT) + std::to_string(i)};
