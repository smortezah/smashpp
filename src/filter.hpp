...
class Filter {
 public:
  ...
  void smooth_seg(std::vector<PosRow>&, std::span<const prc_t>, std::unique_ptr<Param>&, uint8_t,
                  uint64_t&);
 ...
 private:
  ...
  void smooth_seg_win1(std::vector<PosRow>&, std::span<const prc_t>, std::unique_ptr<Param>&,
                       uint8_t);
  template <bool SaveFilter>
  void smooth_seg_rect(std::vector<PosRow>&, std::span<const prc_t>, std::unique_ptr<Param>&,
                       uint8_t);
  template <bool SaveFilter>
  void smooth_seg_non_rect(std::vector<PosRow>&, std::span<const prc_t>, std::unique_ptr<Param>&,
                           uint8_t);
