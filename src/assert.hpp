...
class ValRange {
 public:
  ValRange () = default;
  ValRange (Value min_, Value max_, Value d_, std::string&& l_, 
    std::string&& c_, std::string&& m_, Problem p_) 
    : min(min_), max(max_), def(d_), label(std::move(l_)), 
    criterion(std::move(c_)), initMode(std::move(m_)), problem(p_), 
    inRange(true) {}
  ...
  const auto append_msg = [&](std::string&& msg) {
    ...
    if (initMode == "default")
      message += "Default value "+
        (isFloat ? string_format("%.1f", def) 
                 : std::to_string(def)) + " been set.";
...
class ValSet {
 public:
  ...
  ValSet (const std::vector<Value>& set_, Value d_, std::string&& l_,
    ...
    : set(set_), cmd(c_), def(d_), label(std::move(l_)), 
    initMode(std::move(m_)), problem(p_), inRange(i) {}
