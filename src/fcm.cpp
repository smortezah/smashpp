...
inline void FCM::show_info (const Param& p) const {
  ...
  constexpr uint8_t tblWidth=52;//static_cast<u8>(lblWidth+Ms.size()*colWidth);
  ...
  const auto info_MM = [&](const std::vector<MMPar>& Ms, char c) {
    ...
    for (const auto& e : Ms) {
      ...
      case 'w':
        std::cerr << (e.w==0 ? "0" 
          : "2^" + std::to_string(static_cast<int>(std::log2(e.w))));
        break;
      case 'd':  std::cerr << static_cast<int>(e.d);  break;
      case 'i':  std::cerr << (e.ir==0 ? "reg" 
                             : e.ir==1 ? "inv" : "reg+inv");  break;
      case 'a':  std::cerr << e.alpha;  break;
      case 'g':  std::cerr << e.gamma;  break;
      default:   break;
      }
    }
  };
  const auto info_STMM = [&](const std::vector<MMPar>& Ms, char c) {
    for (const auto& e : Ms) {
      ...
      if (e.child)
        ...
        case 'i':  std::cerr << (e.child->ir==0 ? "reg"
                               : e.child->ir==1 ? "inv" : "reg+inv");  break;
        case 'a':  std::cerr << e.child->alpha;  break;
        case 'g':  std::cerr << e.child->gamma;  break;
        default:   break;
        }
      else
        std::cerr << '-';
  ...
  const auto info_file = [&](char c) {
    ...
    case 't':  std::cerr << p.tarName;  break;
    default:   break;
    }
  };
  const auto rmm_row = [=](std::string&& lbl, char c) {
    label(lbl);    info_MM(rMs, c);                  std::cerr<<'\n';
  };
  const auto rstmm_row = [=](std::string&& lbl, char c) {
    label(lbl);    if (c!='h') info_STMM(rMs, c);    std::cerr<<'\n';
  };
  const auto tmm_row = [=](std::string&& lbl, char c) {
    label(lbl);    info_MM(tMs, c);                  std::cerr<<'\n';
  };
  const auto tstmm_row = [=](std::string&& lbl, char c) {
    label(lbl);    if (c!='h') info_STMM(tMs, c);    std::cerr<<'\n';
  };
  const auto filter_row = [=](std::string&& lbl, char c) {
    label(lbl);    if (c!='h') info_filter(c);       std::cerr<<'\n';
  };
  const auto file_row = 
    [=](std::string&& lbl, std::string&& s1, std::string&& s2) {
    label(lbl);
    if (s1.size() == 1) {
      char c1 = s1[0];
      char c2 = s2[0];
...
inline void FCM::alloc_model () {
  for (const auto& m : rMs) {
    ...
    case Container::SKETCH_8:
      cmls4.emplace_back(std::make_unique<CMLS4>(m.w, m.d));  break;
    case Container::LOG_TABLE_8:
      lgtbl8.emplace_back(std::make_unique<LogTable8>(m.k));  break;
    case Container::TABLE_32:
      tbl32.emplace_back(std::make_unique<Table32>(m.k));  break;
    case Container::TABLE_64:
      tbl64.emplace_back(std::make_unique<Table64>(m.k));  break;
    }
  }
}
...
inline void FCM::store_1 (const Param& p) {
  auto tbl64_iter=std::begin(tbl64);      auto tbl32_iter=std::begin(tbl32);
  auto lgtbl8_iter=std::begin(lgtbl8);    auto cmls4_iter=std::begin(cmls4);
  ...
  for (uint8_t i=0; i!=rMs.size(); ++i) {    // Mask: 1<<2k-1 = 4^k-1
    ...
    case Container::SKETCH_8:
      thrd[i % vThrSz] = std::thread(
        &FCM::store_impl<uint64_t,decltype(cmls4_iter)>, this, std::cref(p.ref),
        (1ull << (2*rMs[i].k)) - 1ull, cmls4_iter++);
      break;
    case Container::LOG_TABLE_8:
      thrd[i % vThrSz] = std::thread(
        &FCM::store_impl<uint32_t,decltype(lgtbl8_iter)>, this,std::cref(p.ref),
        (1ul << (2*rMs[i].k)) - 1ul, lgtbl8_iter++);
      break;
    case Container::TABLE_64:
      thrd[i % vThrSz] = std::thread(
        &FCM::store_impl<uint32_t,decltype(tbl64_iter)>, this, std::cref(p.ref),
        (1ul << (2*rMs[i].k)) - 1ul, tbl64_iter++);
      break;
    case Container::TABLE_32:
      thrd[i % vThrSz] = std::thread(
        &FCM::store_impl<uint32_t,decltype(tbl32_iter)>, this, std::cref(p.ref),
...
void FCM::compress (const Param& p) {
  ...
  if (rMs.size()==1 && rTMsSize==0)  // 1 MM
    switch (rMs[0].cont) {
    case Container::SKETCH_8:     compress_1(p, std::begin(cmls4));   break;
    case Container::LOG_TABLE_8:  compress_1(p, std::begin(lgtbl8));  break;
    case Container::TABLE_32:     compress_1(p, std::begin(tbl32));   break;
    case Container::TABLE_64:     compress_1(p, std::begin(tbl64));   break;
    }
  else
    compress_n(p);

  std::cerr << message << "finished. Average entropy="
...
inline void FCM::compress_1 (const Param& par, ContIter cont) {
  ...
  for (std::vector<char> buffer(FILE_BUF,0); tf.peek()!=EOF;) {
    ...
    for (auto it=std::begin(buffer); it!=std::begin(buffer)+tf.gcount(); ++it) {
      auto c = *it;
      if (c != '\n') {
        ++symsNo;
        prc_t entr;
...
inline void FCM::compress_n (const Param& par) {
  ...
  for (const auto& mm : rMs) {
    ...
    if (mm.child) 
      cp->pp.emplace_back(
        mm.child->alpha, *maskIter++, static_cast<uint8_t>(2*mm.k));
  }
  const auto totalSize = file_size(par.tar);
  std::ifstream tf(par.tar);
  std::ofstream pf(gen_name(par.ID, par.ref, par.tar, Format::PROFILE));
  ...
  for (std::vector<char> buffer(FILE_BUF,0); tf.peek()!=EOF;) {
    ...
    for (auto it=std::begin(buffer); it!=std::begin(buffer)+tf.gcount(); ++it) {
      ...
      if (c != '\n') {
        ...
        auto tbl64_it=std::begin(tbl64);    auto tbl32_it=std::begin(tbl32);
        auto lgtbl8_it=std::begin(lgtbl8);  auto cmls4_it=std::begin(cmls4);
        ...
        const auto ent = entropy(
          std::begin(cp->w), std::begin(cp->probs), std::end(cp->probs));
...
void FCM::self_compress (const Param& p, uint64_t ID) {
  ...
  if (tMs.size()==1 && tTMsSize==0)  // 1 MM
    switch (tMs[0].cont) {
    case Container::SKETCH_8:  self_compress_1(p, std::begin(cmls4), ID); break;
    case Container::LOG_TABLE_8: self_compress_1(p,std::begin(lgtbl8),ID);break;
    case Container::TABLE_32:  self_compress_1(p, std::begin(tbl32), ID); break;
    case Container::TABLE_64:  self_compress_1(p, std::begin(tbl64), ID); break;
    }
  else
...
inline void FCM::self_compress_n (const Param& par, uint64_t ID) {
  ...
  for (const auto& mm : tMs) {
    ...
    if (mm.child) 
      cp->pp.emplace_back(
        mm.child->alpha, *maskIter++, static_cast<uint8_t>(mm.k<<1u));
  }
  const auto totalSize = file_size(par.seq);
...
ContIter cont, uint8_t n, uint64_t& valUpd) const {
  prc_t P;
...
inline void FCM::freqs_ir0 (std::array<OutT,4>& a, ContIter cont, uint64_t l)
const {
  a = {(*cont)->query(l),
       (*cont)->query(l | 1ull),
       (*cont)->query(l | 2ull),
...
//  if (are_all(fFirst, 0)) { // The same as GeCo
  ...
  if (stmm->enabled) {
    if (best==static_cast<uint8_t>(255))
      ...
      miss_stmm(stmm);
      switch (stmm->ir) {
      case 0:  cp->ppIt->config_ir0(best);  break;
      case 1:  cp->ppIt->config_ir1(best);  break;
      case 2:  cp->ppIt->config_ir2(best);  break;
      default:                              break;
      }
    }
  }
...
////    stmm->enabled = true;
////    #ifdef ARRAY_HISTORY
////    std::fill(stmm->begin(history), end(stmm->history), false);
////    #else
////    stmm->history = 0u;
////    #endif
...
#else
template <typename History, typename Value>
inline void FCM::update_hist_stmm (History& hist, Value val, uint32_t mask)
const {
...
inline prc_t FCM::entropy (WIter wFirst, PIter PFirst, PIter PLast) const {
  return std::log2(
    1 / inner_product(PFirst, PLast, wFirst, static_cast<prc_t>(0)));
//  return -std::log2(
...
template <typename ProbParIter>
inline void FCM::update_ctx_ir2 (uint64_t& ctx, uint64_t& ctxIr, ProbParIter pp)
const {
  ctx   = (pp->l & pp->mask) | pp->numSym;
  ctxIr = (pp->revNumSym<<pp->shl) | pp->r;
}