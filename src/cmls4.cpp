...
inline void CMLS4::set_a_b () {
  ...
  std::uniform_int_distribution<uint64_t> uDistB(
    0, (1ull<<uhashShift)-1);
...
inline uint8_t CMLS4::min_log_ctr (uint64_t ctx) const {
...
inline uint8_t CMLS4::read_cell (uint64_t idx) const {
  return CTR[((idx&1ull)<<8u) + sk[idx>>1u]];
////  return CTR[static_cast<u16>(((idx&1ull)<<8u) | sk[idx>>1u])];
}

// Strong 2-universal
inline uint64_t CMLS4::hash (uint8_t i, uint64_t ctx) const {
...
#ifdef DEBUG
uint64_t CMLS4::get_total () const {
  return tot;
}
...
inline void CMLS4::printAB () const {
  ...
  std::cerr.width(w);  cerr<<std::left<<"a";
  std::cerr << "b\n";
  ...
  for (uint8_t i=0; i!=ab.size(); ++i) {
    std::cerr.width(w);  std::cerr<<std::left<<ab[i];
    if (i & 1u)
      std::cerr << '\n';
  }
}
