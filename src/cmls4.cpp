...
void CMLS4::set_a_b() {
  ...
  std::uniform_int_distribution<uint64_t> uDistA(0, (1ull << 63u) - 1);  // k <= 2^63-1
...
uint8_t CMLS4::read_cell(uint64_t idx) const { return CTR[((idx & 1ull) << 8u) + sk[idx >> 1u]]; }
...
auto CMLS4::query_counters(CMLS4::ctx_t l) const -> std::array<CMLS4::val_t, CARDIN> {
  return {query(l), query(l | 1ull), query(l | 2ull), query(l | 3ull)};
}

#ifdef DEBUG
void CMLS4::dump(std::ofstream& ofs) const { ofs.write((const char*)&sk[0], sk.size()); }

void CMLS4::load(std::ifstream& ifs) const { ifs.read((char*)&sk[0], sk.size()); }
