...
# compress_param = ' -rm 4:1000:1:0/0 -rm 14:1000:0:3/10 -c 30 -g 0.95 '

# MUST NOT use '-v' option with AC
compress_param = ' -rm 1:1:0.8/0:0:0 -rm 5:20:0.9/3:20:0.9 '
...
#     execute('sudo chmod -R 777 ' + bin_path)
    ...
    print('Finding similarity between sequences ...')
    ...
    for i in range(first, len(in_file_list)):
        ...
        for j in range(first, len(in_file_list)):
            ...
            if ref != tar:
                execute(ac + ' -rm 5:20:0.9/3:20:0.9 -r ' + ref + ' ' + tar)
                # execute(ac + compress_param + '-r ' + ref + ' ' + tar +
                #         ' > log')
                # log_file = open('log', 'r')
