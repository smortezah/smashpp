...
\begin{table*}[h]
  ...
  \begin{tabularx}{\linewidth}{lrrlX}
    \toprule
    Sequence & \multicolumn{1}{l}{Length} & \multicolumn{1}{l}{Read line} & Kingdom & Description \\
     & \multicolumn{1}{l}{(base)} & \multicolumn{1}{l}{(base)} &  &  \\
    \midrule
    \multicolumn{5}{l}{Synthetic data -- generated by GOOSE toolkit\textsuperscript{a}} \\
    \cmidrule{1-4}
    \textit{Ref}: RefS & 1,500 & 100 & -- & It consists of three segments of 500~base size. \\
    \textit{Tar}: TarS & 1,500 & 100 & -- & To build TarS, segment~I is mutated~2\%, II is inversely repeated and III is duplicated. \\
    \midrule
    \textit{Ref}: RefM & 100,000 & 100 & -- & It has four segments of 25~kilobase size. \\
    \textit{Tar}: TarM & 100,000 & 100 & -- & For building TarM, segment~I of RefM (out of total~four) is inversely repeated, II is mutated~90\%, III is duplicated and IV is mutated~3\%. \\
    \midrule
    \textit{Ref}: RefL & 5,000,000 & 100 & -- & It includes two segments, 2,500,000~bases each. \\
    \textit{Tar}: TarL & 5,000,000 & 100 & -- & Segment~I is inversely repeated and II is mutated~2\% for building TarL. \\
    \midrule
    \textit{Ref}: RefXL & 100,000,000 & 100 & -- & It is made of four segments, 25,000,000~bases each. \\
    \textit{Tar}: TarXL & 100,000,000 & 100 & -- & Segment~I is mutated~1\%, segments~II and III are inversely repeated and segment~IV is duplicated to make TarXL. \\
    \midrule
    \textit{Ref}: RefMut & 60,000 & 100 & -- & It includes 60 segments of 1~kilobase size. \\
    \textit{Tar}: TarMut & 60,000 & 100 & -- & To build TarMut, the first segment~(I) is mutated~1\%, the second segment is mutated~2\%, the third one is mutated~3\%, and so on. \\
    \midrule
    Real data \\
    \cmidrule{1-4}
    \textit{Ref}: PXO99A\textsuperscript{b} & 5,238,555 & 70 & Bacteria & \textit{Xanthomonas oryzae}~pv.~\textit{oryzae} causes the major disease of bacterial blight of rice (\textit{Oryza sativa L.}). \textit{Xanthomonas oryzae}~pv.~\textit{oryzae} PXO99A strain is a 5-azacytidine-resistant derivative of PXO99, which was isolated in Los Baños and classified as Philippine race 6. PXO99A is virulent toward a large number of rice varieties representing diverse genetic sources of resistance~\cite{salzberg2008genome}. \\
    \textit{Tar}: MAFF 311018\textsuperscript{c} & 4,940,217 & 70 & Bacteria & \textit{Xanthomonas oryzae}~pv.~\textit{oryzae} MAFF~311018 is a Japanese race 1 strain~\cite{ochiai2005genome}. \\
    \midrule
    \bottomrule
  \end{tabularx}

\begin{tablenotes}
\item \textsuperscript{a}\url{https://github.com/pratas/goose}
...
conforms to \cite{salzberg2008genome}
\begin{figure*}[!h]
  \centering
...
AML:~acute myeloid leukemia; CMT:~Charcot–Marie–Tooth; CMLS:~Count-Min-Log Sketch; CPU:~central processing unit; FCM:~finite-context model; GB:~gigabyte; GHz:~gigahertz; HTS:~high-throughput sequencing; KB:~kilobyte; MB:~megabyte; RAM:~random access memory; PMP22:~peripheral myelin protein 22; PRNG:~pseudo-random number generator; STMM:~substitutional tolerant Markov model; SVG:~Scalable Vector Graphics; UI:~user interface; WHS:~Wolf–Hirschhorn syndrome.
