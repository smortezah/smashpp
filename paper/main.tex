...
\begin{table*}[h]
  ...
  \begin{tabularx}{\linewidth}{lrrlX}
    \toprule
    Sequence & Length (base) & Read line (base) &  & Description \\
    \midrule
    % \multicolumn{3}{l}{Synthetic data -- generated by GOOSE toolkit\textsuperscript{a}} \\
    % \midrule
    % Ref: RefS & 1,500 & It consists of three segments of size~500 bases. \\
    % Tar: TarS & 1,500 & To build TarS, segment~I is mutated~2\%, II is inversely repeated and III is duplicated. Read lines are 100~base long. \\
    % RefM & 100,000 & TarM & 100,000 & For building TarM, segment~I of RefM (out of total~four) is inversely repeated, II is mutated~90\%, III is duplicated and IV is mutated~3\%. The length of read lines is 100~bases. \\
    % RefL & 5,000,000 & TarL & 5,000,000 & RefL includes two segments, 2,500,000~bases each. Segment~I is inversely repeated and II is mutated~2\% for building TarL. Read lines are of length 100~bases. \\
    ...
    % RefMut & 60,000 & TarMut & 60,000 & RefMut includes 60 segments. To build TarMut, the first segment~(I) is mutated~1\%, the second segment is mutated~2\%, the third one is mutated~3\%, and so on. The length of read lines is 100~bases. \\
    % \midrule
    % Real data \\
    % \midrule
    % PXO99A & 5,313,393 & MAFF 311018 & 5,010,793 & \textit{Xanthomonas oryzae} pv. \textit{oryzae} causes the major disease of bacterial blight of rice (\textit{Oryza sativa L.}). PXO99A strain is a 5-azacytidine-resistant derivative of PXO99, which was isolated in Los Baños and classified as Philippine race 6. PXO99A is virulent toward a large number of rice varieties representing diverse genetic sources of resistance~\cite{salzberg2008genome}.
    % MAFF 311018 is a Japanese race 1 strain~\cite{ochiai2005genome}.
    % \\
    \bottomrule
  \end{tabularx}
