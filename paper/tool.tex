...
\begin{small}
% \begin{longtable}{@{}lp{2.9cm}p{9.5cm}@{}}
% \begin{longtable}{@{}lp{2.9cm}X@{}}
...
%   \midrule
%   \mono{-l} & Integer: $[0, 5]$ & Level of compression (default: 0). \\
%   \midrule
%   \mono{-m} & Integer: $[1, 2^{32}-1]$ & Minimum segment size (default: 50). Only those regions in the reference file that are bigger than this value would be able to be considered for compression. \\
%   % \multicolumn{2}{@{}l@{}}{Minimum segment size (default: 50). Only those regions in the reference file that are bigger than this value would be able to be considered for compression.} \\
%   \midrule
%   \mono{-e} & Float: $[0.0, 100.0]$ & Entropy of `N's (default: 2.0). In implementation of the reference-based compression, we have replaced `N' bases in the references and the targets with `A's and `T's, respectively. On reference-free compression, they are replaced with `A's, in both references and targets. If a user tends to replace `N' bases in a sequence with a normal distribution of `A', `C', `G' and `T's, he/she can employ GOOSE toolkit~\cite{web-goose}. \\
%   \midrule
%   \mono{-n} & Integer: $[1, 8]$ & Number of threads (default: 4). Creating multiple finite-context models and substitutional-tolerant Markov models can be done in a multi-threaded fashion. \\
%   \midrule
%   \mono{-f} & Integer: $[1, 2^{32}-1]$ & Filter size (default: 256). In the process of finding similar regions in the reference and the target sequences, the information content that would be obtained by compression needs to be filtered. \\
%   \midrule
%   \mono{-ft} & Integer/String: \{0$|$rectangular, 1$|$hamming, 2$|$hann, 3$|$blackman, 4$|$triangular, 5$|$welch, 6$|$sine, 7$|$nuttall\} & Filter type/windowing function (default: hann). Besides Hann window that is used by default to smooth the information content (profile), we have implemented several other windowing functions: Blackman~\cite{blackman1959particular}, Hamming~\cite{tukey1949measuring}, Nuttall~\cite{nuttall1981some}, rectangular~\cite{oppenheim1999discrete}, sine~\cite{harris1978use}, triangular~\cite{bartlett1950periodogram} and Welch~\cite{welch1967use} windows. These functions are given by Equation~\ref{eq.filter} and are plotted in Fig.~\ref{fig.filters}.
%   \\
%   \midrule
%   \mono{-fs} & Char/String: \{S$|$small, M$|$medium, L$|$large\} & Filter scale (default: large). \\
%   \midrule
%   \mono{-d} & Integer: $[1, 2^{64}-1]$ & Sampling steps (default: 1). Instead of considering the whole information content, this option can be used to make samples of it. \\
%   \midrule
%   \mono{-th} & Float: $[0.0, 20.0]$ & threshold (default: 1.5). For the purpose of segmenting the filtered information content, the average entropy of reference-based compression is used by default as the threshold, but the threshold can be altered by this option. \\
%   \midrule
%   \mono{-rb} & Integer: $[-2^{15}, 2^{15}-1]$ & Reference beginning guard (default: 0). \\
%   \mono{-re} & Integer: $[-2^{15}, 2^{15}-1]$ & Reference ending guard (default: 0). \\
%   \mono{-tb} & Integer: $[-2^{15}, 2^{15}-1]$ & Target beginning guard (default: 0). \\
%   \mono{-te} & Integer: $[-2^{15}, 2^{15}-1]$ & Target ending guard (default: 0). \\
...
%     \item $a$ (float $>0$): estimator,
%     \item $g$ (float: $[0.0, 1.0)$): forgetting factor,
...
%   \multicolumn{2}{@{}l}{\mono{--version}} & Show version. \\
  \bottomrule
  \vspace{-2mm}
  % \multicolumn{2}{@{}l}{$^*$ Mandatory, $^\dagger$ no input, $\times$ not used} & \\
\end{tabularx}
\end{small}
